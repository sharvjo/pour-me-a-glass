...
const LookupCard = ({ wine, onSave, isSaved, rating, onRate }) => {
  ...
  return (
    <div style={{
      ...
      {/* Body */}
      <div>
        <div style={{ fontFamily: "'Caveat', cursive", fontSize: "13px", fontWeight: "700", color: "#1a2b5e", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "6px" }}>about this wine</div>
...
export default function WinePicker() {
  ...
  const buildLookupAnswerPrompt = (wineDescriptor) => {
    const parts = [];
...
If you genuinely don't recognize the wine and would have to invent facts, return status "unknown" and ask one clarifying question (vintage, region, or producer).
If the descriptor is too vague to answer well (e.g. just "a Cab"), return status "unknown" and ask for what would narrow it down.
...
- "story": 1-2 sentences — something genuinely interesting (producer's story, vintage quirk, why it matters in its category). Not generic.
...
FORMAT UNKNOWN: {"status":"unknown","question":"What would narrow it down? e.g. ..."}`;
  };
...
FORMAT UNREADABLE: {"status":"unreadable"}`;
  ...
  const handleLookupSubmit = async () => {
    ...
    try {
      ...
      if (lookupQuery.trim()) {
        const parsed = await callAPI(buildLookupAnswerPrompt(lookupQuery.trim()), null);
        if (parsed.status === "known") {
          setLookupResult(parsed.wine);
        } else {
          setLookupError(parsed.question || "I couldn't pin that down — try adding the producer or vintage.");
        }
        return;
      }
      ...
      // status === "single"
      const answerParsed = await callAPI(buildLookupAnswerPrompt(idParsed.wine), null);
      if (answerParsed.status === "known") {
        setLookupResult(answerParsed.wine);
      } else {
        setLookupError(answerParsed.question || "Couldn't pull together a confident answer — try adding more detail.");
      }
    } catch {
  ...
  const handleLookupPick = async (wineDescriptor) => {
    ...
    try {
      const parsed = await callAPI(buildLookupAnswerPrompt(wineDescriptor), null);
      if (parsed.status === "known") {
        setLookupResult(parsed.wine);
      } else {
        setLookupError(parsed.question || "Couldn't pull together a confident answer.");
      }
    } catch {
      setLookupError("Something went wrong — try again.");
    } finally {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Lora', serif" }}>
      ...
      <div style={{ maxWidth: "520px", margin: "0 auto", padding: "28px 20px 56px" }}>
        ...
        {mode === "lookup" && (<>
          {/* Photo (optional) */}
          <div onClick={() => lookupFileRef.current.click()} style={{
            ...
            <input ref={lookupFileRef} type="file" accept="image/*" capture="environment" onChange={handleLookupImage} style={{ display: "none" }} />
          </div>
          ...
          {/* Submit / loading / candidates / result / error */}
          {!lookupLoading && !lookupResult && !lookupCandidates && !lookupError && (
            <button
