...
const WineCard = ({ wine, index, onSave, isSaved, selectedTypes, rating, onRate }) => {
  ...
  return (
    <div style={{ position: "relative", background: cardBg, border: `2px solid ${cardBorder}`, borderRadius: "12px", padding: "18px 20px", boxShadow: isDark ? "4px 4px 0px rgba(26,43,94,0.25)" : "2px 2px 0px rgba(0,0,0,0.06)", animation: `fadeUp 0.35s ease ${index * 0.1}s both` }}>
      ...
      <div style={{ position: "absolute", top: "11px", right: "12px", display: "flex", gap: "4px", alignItems: "center" }}>
        <button
      ...
      <div style={{ paddingLeft: "26px", paddingRight: "78px" }}>
...
export default function WinePicker() {
  ...
  const handleRate = (wineKey, newRating) => {
    ...
    if (updated[wineKey] === newRating) {
      // Tapping the active rating clears it
  ...
  const buildRatingContext = () => {
    ...
    const loved = entries.filter(([, r]) => r === "love").map(([k]) => k).slice(0, 12);
    const liked = entries.filter(([, r]) => r === "like").map(([k]) => k).slice(0, 12);
    const disliked = entries.filter(([, r]) => r === "dislike").map(([k]) => k).slice(0, 12);
    const out = [];
    if (loved.length) out.push(`Wines/styles they LOVED previously (lean strongly toward this profile): ${loved.join("; ")}`);
    if (liked.length) out.push(`Wines/styles they LIKED previously (good calibration signal): ${liked.join("; ")}`);
    if (disliked.length) out.push(`Wines/styles they DID NOT like (avoid this profile, do not pick anything similar): ${disliked.join("; ")}`);
    return out.join(". ");
  };
