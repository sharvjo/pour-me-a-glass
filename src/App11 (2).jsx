  ...
  return (
    <div style={{ position: "relative", background: cardBg, border: `2px solid ${cardBorder}`, borderRadius: "12px", padding: "18px 20px", boxShadow: isDark ? "4px 4px 0px rgba(26,43,94,0.25)" : "2px 2px 0px rgba(0,0,0,0.06)", animation: `fadeUp 0.35s ease ${index * 0.1}s both` }}>
      ...
      <div style={{ paddingLeft: "26px" }}>
        ...
        {isStyleCard ? (
          <>
            <div style={{ fontSize: "20px", fontFamily: "'Caveat', cursive", fontWeight: "700", color: isDark ? "#ffffff" : "#1a1208", marginBottom: "2px", lineHeight: 1.2 }}>{wine.style}</div>
            ...
            <div style={{ fontSize: "14px", color: isDark ? "rgba(255,255,255,0.85)" : "#3a2e1e", fontFamily: "'Lora', serif", lineHeight: 1.6, marginBottom: "14px" }}>{wine.description}</div>
            {wine.producers && wine.producers.length > 0 && (
              <div style={{ borderTop: `1.5px dashed ${isDark ? "rgba(255,255,255,0.15)" : C.border}`, paddingTop: "12px" }}>
                <div style={{ fontFamily: "'Caveat', cursive", fontSize: "12px", fontWeight: "700", color: isDark ? "rgba(255,255,255,0.5)" : C.muted, letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: "8px" }}>producers to try</div>
                <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                  {wine.producers.map((p, pi) => (
                    <div key={pi} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      <span style={{ fontFamily: "'Caveat', cursive", fontSize: "13px", fontWeight: "700", color: isDark ? "rgba(255,255,255,0.4)" : C.muted, width: "70px", flexShrink: 0 }}>
                        {p.type === "mainstream" ? "mainstream" : "somm pick"}
                      </span>
                      <span style={{ fontFamily: "'Caveat', cursive", fontSize: "15px", fontWeight: "600", color: isDark ? "#ffffff" : C.text }}>{p.name}</span>
                    </div>
                  ))}
                </div>
              </div>
...
Food pairing is top priority. Each pick is a style/region recommendation, not a single bottle. For each, suggest 4 producers: 2 mainstream (widely available, recognizable) and 2 somm picks (smaller, emerging, more interesting).

The 3 picks must be:
1. tier "easy_find" — an approachable, crowd-pleasing style anyone can find
2. tier "famous" — a well-known classic style that shows up on good restaurant lists
3. tier "sommelier" — a more interesting or unexpected style a wine bar person would recommend

Return ONLY this JSON, no markdown:
{"status":"match","picks":[{"style":"Style Name e.g. Crisp Italian White","region":"grape/region e.g. Vermentino, Sardinia","description":"2 sentences on what it tastes like and why it works for their preferences","tier":"easy_find","producers":[{"name":"Producer Name","type":"mainstream"},{"name":"Producer Name","type":"mainstream"},{"name":"Producer Name","type":"somm"},{"name":"Producer Name","type":"somm"}]},{"style":"...","region":"...","description":"...","tier":"famous","producers":[...]},{"style":"...","region":"...","description":"...","tier":"sommelier","producers":[...]}]}`;
    }
  };
